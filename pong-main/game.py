...
class Game:
    ...
    board_size : int
        size of the game board (board is square)
    max_moves : int
        maximum number of moves allowed in each game
    """
    ...
    def play(self):
        ...
        while True:
            ...
            s = self.permanentAgentRL.center_to_state(self.opponent.centery, self.screen_height, self.bar_height)
            ...
            if self.player_type == 'human':
                ...
                s = self.agentRL.center_to_state(self.player.centery, self.screen_height, self.bar_height)
    ...
    def animation_agentAI(self):
        ...
        if self.player.bottom > self.ball.y:
            self.player.bottom -= self.player_speed
        if self.player.top <= 0:
            self.player.top = 0
        if self.player.bottom >= self.screen_height:
            self.player.bottom = self.screen_height
